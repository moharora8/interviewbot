...
'''
	...
	If you would like TensorFlow to automatically choose an existing and supported device to run the operations 
		in case the specified one doesn't exist, you can set allow_soft_placement to True 
...
@click.option('-inf', '--testing-mode', is_flag=True, help='Flag for testing mode',)

def train(data_corpus, batch_size, num_epochs, learning_rate, testing_mode):

    metadata, trainX, trainY, testX, testY, validX, validY = initial_setup(data_corpus)

    # print(str(len(trainX))) 329 element
    # print(str(len(trainY))) 329 element
    ...
    # print("length->" + str(len(metadata['idx2w'])))
    emb_dim = 1024
    ...
    start_id = src_vocab_size  # 1686
    end_id = src_vocab_size + 1  # 1687
